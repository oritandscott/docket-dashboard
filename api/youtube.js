// Merged replacement for youtube-comments.js + youtube-reply.js, combined
// into one file to stay under Vercel Hobby's 12-serverless-function cap.
// Both only ever run from the dashboard's "Comments" panel and nothing else
// depends on them separately, so merging them is low-risk.
//
// GET  -- (was youtube-comments.js) pulls recent top-level comments from
//         both YouTube channels, plus the title/publish date of the video
//         each was left on. Read-only -- never posts anything.
// POST -- (was youtube-reply.js) posts a single reply to a YouTube comment.
//         Only ever runs when a human clicks Send in the dashboard --
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      return await handlePostReply(req, res);
    }
